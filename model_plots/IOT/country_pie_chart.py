...
@make_facet_grid_plot(
    map_as="dataset",
    encodings=("col", "row"),
    supported_hue_styles=("continuous","discrete"),
    # hue_style="continuous",
    add_guide=False,
    parse_cmap_and_norm_kwargs=False,
    register_as_kind="pie"
)
def pie(
    ds: xr.Dataset,
    hlpr: PlotHelper,
    ...
    **plot_kwargs
    ...
    hlpr.ax.set(xlim=(-limit, limit), ylim=(-limit, limit))
    ...
    for i, country in enumerate(ds.coords[coord_name].data):
        ...
        # Add functions here
        _draw_wedge(hlpr.ax, start, end, length, bar_length, color=cm.map_to_color(ds[hue].sel({coord_name: country}).data))
        # add_flag(ax, flag_x, flag_y, row.country, flag_zoom, angle)

        if (end - start) > 1:
            _add_text(hlpr.ax, text_x, text_y, country, f'{ds[hue].sel({coord_name: country}).data:.3g}', angle)
        start = end

    _dist = ds[y].max().item() - ds[y].min().item()

    _draw_reference_line(hlpr.ax, 0.25*ds[y].max().item(), 0.005 * _dist, inner_padding, color='#F8F1F1')
    _draw_reference_line(hlpr.ax, 0.5*ds[y].max().item(), 0.005 * _dist, inner_padding, color='#F8F1F1')
    _draw_reference_line(hlpr.ax, 0.75*ds[y].max().item(), 0.005 * _dist, inner_padding, color='#F8F1F1')

    hlpr.ax.axis("off")
    hlpr.ax.set_aspect('equal')
    hlpr.ax.set_title(title, x=0.5, y=0.5, va="center", ha="center", linespacing=1.5)